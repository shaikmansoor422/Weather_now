...
function get_Weather_by_location(latitude, longitude) {
    fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current_weather=true&hourly=relative_humidity_2m,wind_speed_10m,wind_direction_10m,precipitation,surface_pressure,cloudcover
`
    )
        .then((response) => response.json())
        .then((data) => {
...
function change_bg_image(data) {
    ...
    if ((time <= 6 || time >= 18) && (data.hourly.relative_humidity_2m[0] >= "15" || data.hourly.relative_humidity_2m[0] <= "22")) {
